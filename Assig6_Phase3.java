...
public class Assig6
{
   ...
   public static void playBuild()
   {
      ...
      // Create highCardGame handler
      CardGameFramework buildGame = new CardGameFramework(numPacksPerDeck, numJokersPerPack, numUnusedCardsPerPack,
            unusedCardsPerPack, NUM_PLAYERS, NUM_CARDS_PER_HAND);
...
class Controller
{
   ...
   private void playBuild()
   {
      ...
      char[] playableValues = new char[4]; 
      setPlayableValues(playableValues, leftPlayCard, rightPlayCard);
   ...
   // Timer to allow for game to timeout if ignored.
      ...
      while (((((new Date().getTime()) / 1000) - activityTimer) < 300) && model.getHand(humanHandID).getNumCards() > 0)
      {
          ...
          * 
          * Player Turn
          * user selects a card from hand
          * if user selects a different hand card, deselect first card
          * user selects a playCard in playArea (left or right)
          * check if hand card value can be played on playCard
          * if (valid play)
          * start Computer Turn
          * 
          * Computer Turn
   ...
   private void setPlayableValues(char[] cardValues, Card leftCard, Card rightCard)
   {
	   switch (leftCard.getValue()) {
       case 'A':
