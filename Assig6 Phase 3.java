...
high-card game.  This is the third phase.  In this phase, we will write new code using a MVC pattern to create a 
timed high-card game.  For our MVC pattern, class CardGameFramework will function as our model, class CardTable
will function as our view, and class Controller will function as our controller.
----------------------------------------------------------------------------------------------------------------- */
...
public class Assig6
{
   ...
   public static void playHighCard()
   {
      ...
      // Creates game timer and then starts it
      GameTimer timer = new GameTimer();
      timer.startTimer();
      // Create game table as in previous phases.
      CardTable myCardTable =
            new CardTable("CardTable", NUM_CARDS_PER_HAND, NUM_PLAYERS, Controller.Games.highCard, timer);
      // Deal cards to players.
      new Controller(myCardTable, highCardGame, Controller.Games.highCard);
   }
   ...
   public static void playBuild()
   {
      ...
      timer.startTimer();
      CardTable myCardTable =
            new CardTable("CardTable", NUM_CARDS_PER_HAND, NUM_PLAYERS, Controller.Games.build, timer);
      new Controller(myCardTable, buildGame, Controller.Games.build);
   }
...
class CardTable extends JFrame
{
   ...
   private JPanel pnlComputerHand, pnlHumanHand, pnlPlayArea, pnlControls, pnlTimer;
   private JLabel[] computerLabels;
   private JLabel[] humanLabels;
   private JLabel[] playedCardLabels;
   private JLabel[] playLabelText;
   private static JButton toggleTimerButton;
   ...
   public CardTable(String title, int numCardsPerHand, int numPlayers, GameTimer timer)
   {
      ...
      pnlControls.setBorder(BorderFactory.createTitledBorder("Timer"));
      ...
      //timer button
      toggleTimerButton = new JButton("Stop Timer");
      pnlControls.add(toggleTimerButton);
   ...
   public CardTable(String title, int numCardsPerHand, int numPlayers,
         Controller.Games game, GameTimer timer)
   {
      this(title, numCardsPerHand, numPlayers, timer);
      currentGame = game;
   }
   ...
   {
      // Remove play area, it is no longer needed.
      remove(pnlPlayArea);
      
      // Clear the computer's hand
      pnlComputerHand.removeAll();
      
      // Set up computer's hand panel to display score
      pnlComputerHand.setBorder(BorderFactory.createTitledBorder("Computer Score"));
      
      JLabel computerWinnings = new JLabel("Computer turns passed: " + Integer.toString(computerTurns), JLabel.CENTER);
      
      // Make a readable font.
      Font f = new Font("SansSerif",Font.PLAIN,20);
      computerWinnings.setFont(f);
      
      pnlComputerHand.add(computerWinnings);
      
      // Generate results message for the player. Win or Loss.
      String results = "";
      ...
      // And print player's score
      pnlHumanHand.removeAll();
      pnlHumanHand.setBorder(BorderFactory.createTitledBorder("Your Score"));
      
      String message = results + " Your turns passed: " + Integer.toString(playerTurns);
      JLabel humanWinnings = new JLabel(message, JLabel.CENTER);
      
      // Reuse readable font.
      humanWinnings.setFont(f);
      
      pnlHumanHand.add(humanWinnings);
   ...
   // Create UI and buttons for player's cards.
   ...
   // Update the card options in the players hand, while high lighting a newly selected card.
   public void updatePlayerHandPanel(Hand playerHand, int selectIndex)
   {
      // player's hand
      pnlHumanHand.removeAll();
      humanLabels = new JLabel[playerHand.getNumCards()];
      for (int i = 0; i < humanLabels.length; i++)
      {
         ...
         // if the user selected a card, changed their mind, and selected another, this selects the new card
         if(i == selectIndex)
         {
   ...
   // Allocates more space for the player's hand by one 'row' of cards
   public void increasePlayerHandSpace()
   {
      pnlHumanHand.setPreferredSize(new Dimension(600,pnlHumanHand.getHeight()+107));
   }
   
   public void TimerListener(ActionListener l)
   {
      toggleTimerButton.setCursor(new Cursor(Cursor.HAND_CURSOR));
      toggleTimerButton.addActionListener(l);
   }
   
   static boolean setTimerButtonText(String text)
   {
      if(text == null)
         return false;
      
      toggleTimerButton.setText(text);
      return true;
   }
}
...
class Controller
{
   ...
   public Controller(CardTable view, CardGameFramework model, Games game)
   {
      this.view = view;
      this.model = model;
      this.view.TimerListener(new TimerListener());
   ...
   {    
      ...
      // Memo to track if the user changed their mind about which card to play.
      int cardIDMemo = playedCardID;
      
      // Flag to track if the game is done.
      ...
      while (((((new Date().getTime()) / 1000) - activityTimer) < 300) && 
               model.getHand(humanHandID).getNumCards() > 0 &&
               !isGameFinished)
      {
         // Memorize the first playedCardID change.
         if(playedCardID != -1 && cardIDMemo == -1)
         {
            cardIDMemo = playedCardID;
         }
         
         // If a different card is selected from the hand.
         if(cardIDMemo != -1 && cardIDMemo != playedCardID)
         {
            view.updatePlayerHandPanel(model.getHand(humanHandID), playedCardID);
            cardIDMemo = playedCardID;
         }
         ...
         if (isUnableToPlay == true)
         {
            ...
            // If we need another row for the player's hand, increase the panel size. (10 cards per row)
            if (model.getHand(humanHandID).getNumCards() > 1 && 
                  model.getHand(humanHandID).getNumCards() % 10 == 1)
            {
               view.increasePlayerHandSpace();
            }
         }
...
class TimerListener implements ActionListener
{
   ...
   public void actionPerformed(ActionEvent e)
   {
      ...
      {
         playing = false;
         CardTable.setTimerButtonText("Start Timer");
      }
      else
      {
         playing = true;
         CardTable.setTimerButtonText("Stop Timer");
      }
      timer.toggleTimer();
   }  
}
